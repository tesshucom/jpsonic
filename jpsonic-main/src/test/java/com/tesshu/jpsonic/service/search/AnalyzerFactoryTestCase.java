...
public class AnalyzerFactoryTestCase extends TestCase {
  ...
  public void testProperTokenizer() {
    ...
    // [keyword/no tokenize]
    // (should not GROUP)
    // GENRE FOLDER MEDIA_TYPE 
    // (Supplement of Japanese search. Data is not necessarily included in all docs.)
    // ARTIST_FULL ARTIST_READING_HIRAGANA ALBUM_FULL 

    // [japanese/tokenize]
    // TITLE ALBUM ARTIST ARTIST_READING

    // no touch -> [japanese/tokenyze]
    // ID
    // FOLDER_ID
    // YEAR

    /*
     * [keyword/no tokenize](should not GROUP)
     * 
     * GENRE FOLDER MEDIA_TYPE 
     * -> should not GROUP
     * 
  ...
  public void testProperCase() {
    ...
    assertTrue("GENRE", toTermString(FieldNames.GENRE, query).get(0).equals("abc"));
    assertTrue("FOLDER", toTermString(FieldNames.FOLDER, query).get(0).equals("ABC"));
    assertTrue("MEDIA_TYPE", toTermString(FieldNames.MEDIA_TYPE, query).get(0).equals("abc"));
    assertTrue("ARTIST_FULL", toTermString(FieldNames.ARTIST_FULL, query).get(0).equals("abc"));
    assertTrue("ARTIST_READING_HIRAGANA", toTermString(FieldNames.ARTIST_READING_HIRAGANA, query).get(0).equals("abc"));
    assertTrue("ALBUM_FULL", toTermString(FieldNames.ALBUM_FULL, query).get(0).equals("abc"));

    assertTrue("TITLE", toTermString(FieldNames.TITLE, query).get(0).equals("abc"));
    assertTrue("ALBUM", toTermString(FieldNames.ALBUM, query).get(0).equals("abc"));
    assertTrue("ARTIST", toTermString(FieldNames.ARTIST, query).get(0).equals("abc"));
    assertTrue("ARTIST_READING", toTermString(FieldNames.ARTIST_READING, query).get(0).equals("abc"));
    
    assertTrue("ID", toTermString(FieldNames.ID, query).get(0).equals("abc"));
    assertTrue("FOLDER_ID", toTermString(FieldNames.FOLDER_ID, query).get(0).equals("abc"));
    assertTrue("YEAR", toTermString(FieldNames.YEAR, query).get(0).equals("abc"));
  ...
  /*
   * Analysis accuracy of katakana does not matter so much.
   */
   ...
   * In case of only hiragana,
   * ARTIST_READING_HIRAGANA etc. are registered.
   * 
   * In addition, the artist name full registration of ARTIST_FULL
   * because of its importance.
   * 
   *  - the data size is relatively large.
   *  - Japanese names are often difficult to tokenize
   */
   ...
   * Month and year have important meanings and are often included in the song title.
   */
  ...
  public void testGreekAcute2() {
    String query = "ΆάΈέΉήΊίΐΌόΎύΰϓΏώ";
    List<String> terms = toTermString(
  ...
  public static List<String> toTermString(String field, String str) {
    ...
    try {
      TokenStream stream = AnalyzerFactory
          .getInstance().getAnalyzer().tokenStream(field, new StringReader(str));
      stream.reset();
