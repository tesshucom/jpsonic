...
class SearchServiceTest {
    ...
    class GeneralTest extends AbstractNeedsScan {
        ...
        void testSearchTypical() throws IOException {
            ...
            long l = result
                ...
                .count();
            assertEquals(3L, l,
                    "(33) Artist whose name contains \\\"_ID3_ARTIST_\\\" is 3 records.");
            ...
            assertEquals(1, result.getMediaFiles().size(),
                    "(37) Can the normal case be implemented.");
        ...
        void testGenre() {
            ...
            List<MediaFile> albums = searchService
        ...
        void testGenreMaster() {
            ...
            assertEquals(1L,
                    genres
                        .stream()
                        .filter(g -> "Gothik Folk Psychobilly".equals(g.getName()))
                        .count(),
        ...
        void testToPreAnalyzedGenres() {
            ...
            List<String> preAnalyzedGenres = indexManager
                .toPreAnalyzedGenres(
                        Arrays.asList("Baroque Instrumental", "Gothik Folk Psychobilly"), true);
     ...
     */
    // @Nested Can't seem to run on current maven-enforcer. Executable as a single
    // class.
    @Order(2)
    ...
    class SpecialPathTest extends AbstractNeedsScan {
            ...
            if (isEmpty(musicFolders)) {
                musicFolders = Arrays
                    .asList(new MusicFolder(1,
                            resolveBaseMediaPath("Search/SpecialPath/accessible"), "accessible",
                            true, now(), 0, false),
                            new MusicFolder(2,
                                    resolveBaseMediaPath("Search/SpecialPath/accessible's"),
        ...
        void testSpecialCharactersInDirName() {
            ...
            // ALL Songs
            List<MediaFile> randomAlbums = searchService
                .getRandomAlbums(Integer.MAX_VALUE, folders);
    ...
    /**
     * Test cases related to #1142. The filter is not properly applied when
     * analyzing the query,
     * <p>
     * In the process of hardening the Analyzer implementation, this problem is
     * solved side by side.
     */
    @Nested
    @Order(4)
    class StartWithStopwardsTest extends AbstractNeedsScan {
        ...
        public List<MusicFolder> getMusicFolders() {
            if (isEmpty(musicFolders)) {
                musicFolders = Arrays
                    .asList(new MusicFolder(1, resolveBaseMediaPath("Search/StartWithStopwards"),
                            "accessible", true, now(), 1, false));
            }
            return musicFolders;
        }
        ...
        void testStartWithStopwards() throws IOException {
            ...
            criteria = director
                .construct("williams", offset, count, false, folders, IndexType.ARTIST_ID3);
            ...
            criteria = director
                .construct("willi", offset, count, false, folders, IndexType.ARTIST_ID3);
            result = searchService.search(criteria);
            // 3.x -> 8.x : Normal forward matching => This case does not hit because it is
            // a phrase search rather
            // than
            // a term prefix match.
            ...
            assertEquals(0, result.getTotalHits(), "Theater hit by \"thea\" "); // => This case does
                                                                                // not hit
                                                                                // because
