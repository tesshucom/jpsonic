...
public class AnalyzerTestCase extends TestCase {
  ...
  public void testResource() {
    ...
    // title
    List<String> terms = AnalyzerUtil
        .toTermString(analyzer, "Bach: Goldberg Variations, BWV 988 - Aria");
    ...
    // album
    terms = AnalyzerUtil
        .toTermString(analyzer, "_ID3_ALBUM_ Bach: Goldberg Variations, Canons [Disc 1]");
    assertEquals(9, terms.size());
    /*
     * (lucene 3.0) id3_album
     * 
     * (UAX#29) id 3 album
     */
    assertEquals("id", terms.get(0));
    ...
    // title
    terms = AnalyzerUtil
        .toTermString(analyzer, "Bach: Goldberg Variations, BWV 988 - Variatio 1 A 1 Clav.");
    assertEquals(9, terms.size());
    assertEquals("bach", terms.get(0));
    assertEquals("goldberg", terms.get(1));
    assertEquals("variations", terms.get(2));
  ...
  public void testNumeral() {
    List<String> terms = AnalyzerUtil
        .toTermString(analyzer, "books boxes cities leaves men glasses");
    assertEquals(6, terms.size());
  ...
  /*
   * air -> jp The case is changed because syntax processing of kanji is possible.
  ...
  /*
   * air -> jp Airsonic removes all common stops. Jpsonic removes only articles.
   */
  public void testStopward() {
    ...
    terms = AnalyzerUtil.toTermString(analyzer,
        "and are as at be but by for if in into is it no not of "
        + "on or such that their then there these they this to was will with");
    ...
    terms = AnalyzerUtil.toTermString(analyzer,
          ...
          + "それぞれ または お ほど ものの に対する ほとんど と共に といった です とも"
           + " ところ ここ");// 108 (In Buddhism it is the number of troubles..)
    assertEquals(0, terms.size());
   ...
   * is necessary. Dedicated parsing is required for complete processing.
   * 
   * In reality, as Jpsonic is doing, I think that it is bette to use extension
   * tags that are useful for analysis apart from the display character string.
   * 
   * ex)Julius Cæsar
   * 
   * Register "Julius Caesar" in the SORT tag and use it for searching and
   * indexing.
   */
  ...
  /*
   * In UAX # 29, determination of non-practical word boundaries is not
   * considered. Languages ​​that use special strings require practical
   * verification.
   */
