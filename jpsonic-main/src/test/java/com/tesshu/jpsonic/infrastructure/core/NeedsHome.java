 ...
 *
 * <p>
 * The extension also prepares the {@code transcoding} directory under the
 * temporary home so that components depending on external tools (such as
 * {@code ffmpeg} or {@code ffprobe}) can operate in a controlled and
 * test‑specific environment. When tests need to use a specific ffmpeg
 * installation, provide its directory via the system property
 * {@code -Djpsonic.transcodePath=C:\path\to\ffmpeg\bin}.
 * </p>
 *
 * <p>
