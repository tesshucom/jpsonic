...
public class QueryFactory {
    ...
    public @NonNull Query createFolderQuery(@NonNull Boolean isId3,
        ...
        folders
            .stream()
            .map(isId3 ? this::toFolderIdQuery : this::toFolderPathQuery)
    ...
    private @NonNull Query createYearRangeQuery(@Nullable Integer from, @Nullable Integer to) {
        return IntPoint
            .newRangeQuery(FieldNamesConstants.YEAR, isEmpty(from) ? Integer.MIN_VALUE : from,
    ...
    /**
     * Exclude the search field from the specified field depending on the condition.
     * If necessary, exclude fields related to Composer from the fields to be
     * searched. Also, unnecessary fields are excluded according to the value of
     * Index Scheme.
     *
     * @param fields          Field to search
     * @param includeComposer Whether to include fields related to Composer in the
     *                        search. The judgment method may differ depending on
     *                        the protocol. In the case of HTTP, personal settings
     *                        are also considered. For UPnP, follow server settings.
     *
    ...
    private Optional<Query> createFieldQuery(@NonNull String fieldName, @NonNull String queryString)
        ...
        try (TokenStream stream = analyzerFactory
            .getAnalyzer()
            .tokenStream(isRomanizedField ? FieldNamesConstants.ARTIST_READING : fieldName,
                    queryString)) {
            stream.reset();
    ...
    private Optional<Query> createPhraseQuery(List<String> fieldNames, boolean includeComposer,
        ...
        for (String fieldName : targetFields) {
            ...
            query.ifPresent(q -> {
                fieldQuerys
                    .add(indexType.getBoosts().containsKey(fieldName)
    ...
    public Optional<Query> createPhraseQuery(List<String> targetFields, @NonNull String queryString,
        ...
        return createPhraseQuery(targetFields, settingsService.isSearchComposer(), queryString,
                indexType);
    }
    ...
    public Query searchByPhrase(@NonNull String searchInput, boolean includeComposer,
        ...
        Optional<Query> multiFieldQuery = createPhraseQuery(indexType.getFields(), searchInput,
                indexType);
    ...
    public Query getRandomSongs(@NonNull RandomSearchCriteria criteria) throws IOException {
        ...
        // Unanalyzed field
        query
            .add(new TermQuery(new Term(FieldNamesConstants.MEDIA_TYPE, MediaType.MUSIC.name())),
                    Occur.MUST);
            ...
            for (String genre : criteria.getGenres()) {
                try (TokenStream stream = analyzerFactory
                    ...
                    while (stream.incrementToken()) {
                        String token = stream.getAttribute(CharTermAttribute.class).toString();
                        genreQuery
                            .add(new TermQuery(new Term(FieldNamesConstants.GENRE, token)),
                                    Occur.SHOULD);
                    }
                }
            }
    ...
    public Query getRandomSongs(@NonNull List<MusicFolder> musicFolders, String... genres) {
        ...
        if (genres.length > 0) {
            ...
            Stream
                .of(genres)
                .forEach(genre -> genreQueryBuilder
    ...
    public Query getRandomAlbums(@NonNull List<MusicFolder> musicFolders) {
        return new BooleanQuery.Builder()
            .add(createFolderQuery(false, musicFolders), Occur.SHOULD)
            .build();
    }
    ...
    public Query getRandomAlbumsId3(@NonNull List<MusicFolder> musicFolders) {
        return new BooleanQuery.Builder()
            .add(createFolderQuery(true, musicFolders), Occur.SHOULD)
            .build();
    }
     ...
     */
    @SuppressWarnings("PMD.AvoidInstantiatingObjectsInLoops") // (TermQuery, Term) Not reusable
    public Query getAlbumId3sByGenres(@Nullable String genres,
            @NonNull List<MusicFolder> musicFolders) throws IOException {
        ...
        if (!isEmpty(genres)) {
            ...
            try (TokenStream stream = analyzerFactory
                .getAnalyzer()
                .tokenStream(FieldNamesConstants.GENRE, genres)) {
                stream.reset();
                while (stream.incrementToken()) {
                    genreQuery
                        .add(new TermQuery(new Term(FieldNamesConstants.GENRE,
        ...
        musicFolders
            .forEach(musicFolder -> folderQuery
                .add(new TermQuery(
                        new Term(FieldNamesConstants.FOLDER_ID, musicFolder.getId().toString())),
                        Occur.SHOULD));
    ...
    /**
     * Query generation expression extracted from
     * {@link com.tesshu.jpsonic.service.SearchService#getSongsByGenre(String, int, int, List)}
     * Query generation expression extracted from
    ...
    public Query getMediasByGenres(@Nullable String genres, @NonNull List<MusicFolder> musicFolders)
        ...
        musicFolders
            .forEach(musicFolder -> folderQuery
                .add(new TermQuery(
                        new Term(FieldNamesConstants.FOLDER, musicFolder.getPathString())),
                        Occur.SHOULD));
        query.add(folderQuery.build(), Occur.MUST);
    ...
    public Query getAlbumId3GenreCount(@NonNull String genre, List<MusicFolder> folders) {
        return new BooleanQuery.Builder()
            .add(createFolderQuery(true, folders), Occur.MUST)
    ...
    private Query getTypesQuery(MediaType... types) {
        ...
        Stream
            .of(types)
            .forEach(type -> typeQuery
                .add(new TermQuery(new Term(FieldNamesConstants.MEDIA_TYPE, type.name())),
                        Occur.SHOULD));
    ...
    public Query getSongGenreCount(@NonNull String genre, List<MusicFolder> folders,
            MediaType... types) {
        return new BooleanQuery.Builder()
            .add(createFolderQuery(false, folders), Occur.MUST)
            .add(new TermQuery(new Term(FieldNamesConstants.GENRE, genre)), Occur.MUST)
            .add(getTypesQuery(types), Occur.MUST)
            .build();
    }
    ...
    public Query getAlbumChildren(Album album, String genre, List<MusicFolder> folders,
        ...
        return new BooleanQuery.Builder()
            ...
            .add(new TermQuery(new Term(FieldNamesConstants.GENRE, genre)), Occur.MUST)
            .add(createFolderQuery(false, folders), Occur.MUST)
            .add(typeQuery.build(), Occur.MUST)
            .build();
