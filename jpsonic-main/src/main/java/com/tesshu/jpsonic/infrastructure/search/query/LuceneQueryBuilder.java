...
public class LuceneQueryBuilder {
     ...
     */
    public Query buildFolderQuery(boolean isId3, @Nonnull List<MusicFolder> folders) {
     ...
     * @return a BooleanQuery containing the folder conditions
     */
    public Query buildFolderQuery(boolean isId3,
            Collection<com.tesshu.jpsonic.domain.model.MusicFolder> folders) {
        String field = isId3 ? FieldNamesConstants.FOLDER_ID : FieldNamesConstants.FOLDER;
        BooleanQuery.Builder builder = new BooleanQuery.Builder();
        folders
            .stream()
            .map(folder -> isId3 ? Integer.toString(folder.id()) : folder.pathString())
            .map(value -> new TermQuery(new Term(field, value)))
            .forEach(query -> builder.add(query, BooleanClause.Occur.SHOULD));
        return builder.build();
    }
    ...
    public Optional<Query> buildFieldQuery(@Nonnull String fieldName, @Nonnull String queryString) {
        ...
        List<String> tokens;
        try {
            tokens = tokenize(analyzedField, queryString);
            if (tokens.isEmpty()) {
