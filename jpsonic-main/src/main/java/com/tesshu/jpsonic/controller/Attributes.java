...
public final class Attributes {
    ...
    public enum Redirect {
        ...
        BINDING_RESULT("org.springframework.validation.BindingResult.command"), COMMAND("command"),
        ERROR("error"), MODEL("model"), NEW_TRANSCODING("newTranscoding"),
        RELOAD_FLAG("settings_reload"), TOAST_FLAG("settings_toast"), USER_INDEX("userIndex"),
        PLAYER_ID("playerId");
    ...
    public enum Request {
        ACTION(NameConstants.ACTION), ADD(NameConstants.ADD),
        ADD_TO_PLAYLIST(NameConstants.ADD_TO_PLAYLIST), ADMIN_ROLE(NameConstants.ADMIN_ROLE),
        ALBUM(NameConstants.ALBUM), ALBUM_COUNT(NameConstants.ALBUM_COUNT),
        ALBUM_OFFSET(NameConstants.ALBUM_OFFSET), ANY(NameConstants.ANY),
        ARTIST(NameConstants.ARTIST), ARTIST_COUNT(NameConstants.ARTIST_COUNT),
        ARTIST_OFFSET(NameConstants.ARTIST_OFFSET), BITRATE(NameConstants.BITRATE),
        C(NameConstants.C), CALLBACK(NameConstants.CALLBACK), CHANNEL_ID(NameConstants.CHANNEL_ID),
        CLONE(NameConstants.CLONE), COMMENT(NameConstants.COMMENT),
        COMMENT_ROLE(NameConstants.COMMENT_ROLE), COUNT(NameConstants.COUNT),
        COVER_ART_ROLE(NameConstants.COVER_ART_ROLE), CURRENT(NameConstants.CURRENT),
        DECADE(NameConstants.DECADE), DEFAULT_ACTIVE(NameConstants.DEFAULT_ACTIVE),
        DELETE(NameConstants.DELETE), DELETE_CHANNEL(NameConstants.DELETE_CHANNEL),
        DELETE_EPISODE(NameConstants.DELETE_EPISODE), DELETE_EXPIRED(NameConstants.DELETE_EXPIRED),
        ...
        DLNA_RANDOM_SONG_BY_FOLDER_ARTIST_VISIBLE(
                NameConstants.DLNA_RANDOM_SONG_BY_FOLDER_ARTIST_VISIBLE),
        ...
        DOWNLOAD_ROLE(NameConstants.DOWNLOAD_ROLE), DURATION(NameConstants.DURATION),
        EMAIL(NameConstants.EMAIL), ENABLED(NameConstants.ENABLED), ERROR(NameConstants.ERROR),
        EXPIRE_IN(NameConstants.EXPIREIN), EXPIRES(NameConstants.EXPIRES), F(NameConstants.F),
        FORCE_CUSTOM(NameConstants.FORCE_CUSTOM), FORMAT(NameConstants.FORMAT),
        FROM_YEAR(NameConstants.FROM_YEAR),
        G_RECAPTCHA_RESPONSE(NameConstants.G_RECAPTCHA_RESPONSE), GAIN(NameConstants.GAIN),
        GENRE(NameConstants.GENRE), HIDE(NameConstants.HIDE), HLS(NameConstants.HLS),
        HLS_COMMAND(NameConstants.HLS_COMMAND), HOMEPAGE_URL(NameConstants.HOMEPAGE_URL),
        I(NameConstants.I), ID(NameConstants.ID),
        IF_MODIFIED_SINCE(NameConstants.IF_MODIFIED_SINCE),
        INCLUDE_EPISODES(NameConstants.INCLUDE_EPISODES),
        INCLUDE_NOT_PRESENT(NameConstants.INCLUDE_NOT_PRESENT), INDEX(NameConstants.INDEX),
        IS_REST(NameConstants.IS_REST), J_PASSWORD(NameConstants.J_PASSWORD),
        J_USERNAME(NameConstants.J_USERNAME), LDAP_AUTHENTICATED(NameConstants.LDAP_AUTHENTICATED),
        LIST_OFFSET(NameConstants.LIST_OFFSET), LIST_TYPE(NameConstants.LIST_TYPE),
        LOGOUT(NameConstants.LOGOUT), MAX_BIT_RATE(NameConstants.MAX_BIT_RATE),
        MUSIC_FOLDER_ID(NameConstants.MUSIC_FOLDER_ID), NAME(NameConstants.NAME),
        OFFSET(NameConstants.OFFSET), OFFSET_SECONDS(NameConstants.OFFSET_SECONDS),
        P(NameConstants.P), PASSWORD(NameConstants.PASSWORD), PATH(NameConstants.PATH),
        PLAYER(NameConstants.PLAYER), PLAYLIST(NameConstants.PLAYLIST),
        ...
        RATING(NameConstants.RATING), REFRESH(NameConstants.REFRESH), RESET(NameConstants.RESET),
        S(NameConstants.S), SETTINGS_ROLE(NameConstants.SETTINGS_ROLE),
        ...
        STEP1(NameConstants.STEP1), STEP2(NameConstants.STEP2),
        ...
        TARGET_FORMAT(NameConstants.TARGET_FORMAT), TIME_OFFSET(NameConstants.TIME_OFFSET),
        TITLE(NameConstants.TITLE), TO_YEAR(NameConstants.TO_YEAR), TYPE(NameConstants.TYPE),
        U(NameConstants.U), UPLOAD_ROLE(NameConstants.UPLOAD_ROLE), URL(NameConstants.URL),
        USER(NameConstants.USER), USER_INDEX(NameConstants.USERINDEX),
        USER_NAME(NameConstants.USERNAME), USERNAME_OR_EMAIL(NameConstants.USERNAME_OR_EMAIL),
        V(NameConstants.V), VIEW_AS_LIST(NameConstants.VIEW_AS_LIST), X(NameConstants.X);
